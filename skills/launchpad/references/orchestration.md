...
- A multi-agent run costs roughly **15× the tokens** of a single-agent pass. Token usage
  alone explains most of the variance in what multi-agent work costs.
...
- Multi-agent **hurts** on tightly-coupled work: shared context, many dependencies between
  steps, or several agents editing the same files (last write wins → lost work).
...
- **Parallelism:** there are **3 or more genuinely independent** subtasks that don't share
...
A subagent inherits **no** parent conversation, memory, or system prompt — only the project
`CLAUDE.md` and the prompt you give it. Vague delegation is the top cause of duplicated work,
gaps, and wrong-tool use. Every delegation states, explicitly:

1. **Objective** — the one concrete outcome, in a sentence.
2. **Scope / boundaries** — what's in, what's out, what not to touch.
3. **Known context** — the relevant `MEMORY.md` / `ERRORS.md` / `LEARNINGS.md` entries,
   pasted in (the agent can't read them itself). Plus file paths and any error text.
4. **Output format** — exactly what to return (a list, a diff, a verdict, a structured
   object), so results compose without rework.
...
- **Haiku** — mechanical, high-volume, or classification work (formatting, log triage,
  simple lookups).
- **Sonnet** — most implementation, search, and review.
- **Opus** — hard reasoning: architecture, synthesis of many results, adversarial judging,
  the trickiest bugs.
...
- Run **3–5 agents in parallel** for most fan-out. More than that rarely improves results and
  multiplies cost.
- Hard ceiling is roughly **a dozen or so** concurrent agents; don't design for more by hand.
...
- Custom subagents **do** inherit the project `CLAUDE.md`.
- The built-in **Explore** and **Plan** agents **skip** `CLAUDE.md` (and git status) to stay
  cheap. So if a load-bearing rule must hold inside one of those, **restate it in the spawn
  prompt** — don't assume it carried over.
...
- Do one "role" at a time — e.g. implement, then *as a fresh-eyed reviewer* critique, then
  reconcile. Council runs the same way: five advisors in turn, then the chair.
...
1. Is this a consequential, hard-to-reverse **decision** (not a coding task)? → **Council**.
...
6. 10+ targets or multi-module program? → **Assembly Line / Program**, handed to a Workflow.
