...
This project was built under the **provisional working name `claunchpad`**. The final
...
1. **Folder names** (two):
   - `claunchpad/`  →  `<final-name>/`            (project root)
   - `claunchpad/skills/claunchpad/`  →  `claunchpad/skills/<final-name>/`  (skill folder, in
...
2. **In-file occurrences** of the working name. Replace whole-word, case-aware:
   - `claunchpad`  →  `<final-name>`        (lowercase: frontmatter `name:`, paths, prose)
   - `Claunchpad`  →  `<Final-Name>`        (Title case: headings, README)
   - `LAUNCHPAD`  →  `<FINAL-NAME>`        (if any all-caps usage)

   Files that mention it: `README.md`, `.pcc.json`, `skills/README.md`,
   `skills/claunchpad/SKILL.md` (frontmatter `name:` + body), the four `references/*.md`,
...
- No stray "claunchpad" remains: search the tree for it and confirm only intended hits.
